#!/usr/bin/env python3
"""Fetch Munich coworking spaces from OpenStreetMap Overpass and write GeoJSON."""
...
(
  ...
  relation["office"="coworking"]({bbox});
  node["coworking"]({bbox});
  way["coworking"]({bbox});
  relation["coworking"]({bbox});
  node["name"~"cowork|co-working|co working",i]({bbox});
  way["name"~"cowork|co-working|co working",i]({bbox});
  relation["name"~"cowork|co-working|co working",i]({bbox});
);
out center tags;
""".strip()
...
def coworking_type(tags: dict[str, Any]) -> str:
    value = first_tag(tags, "coworking")
    if value:
        return value

    value = first_tag(tags, "office", "amenity")
    if value in {"coworking", "coworking_space"}:
        return value

    name = first_tag(tags, "name").lower()
    if "cowork" in name or "co-working" in name or "co working" in name:
        return "name_match"
...
def element_to_feature(element: dict[str, Any]) -> dict[str, Any] | None:
    ...
    properties = {
        ...
        or "Unnamed coworking space",
        "category": "Coworking",
        "coworking_type": coworking_type(tags),
...
def to_geojson(overpass_data: dict[str, Any]) -> dict[str, Any]:
    ...
    features.sort(
        key=lambda feature: (
            ...
            str(feature["properties"]["coworking_type"]).lower(),
...
def default_output_path() -> Path:
    ...
    return (
        ...
        / "data"
        / "city-intelligence"
        / "munich-coworking.geojson"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch real Munich coworking data from OpenStreetMap Overpass."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> int:
    ...
    print(f"Wrote {len(geojson['features'])} coworking spaces to {args.output}")
    return 0
