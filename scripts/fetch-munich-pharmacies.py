#!/usr/bin/env python3
"""Fetch Munich pharmacies from OpenStreetMap Overpass and write GeoJSON."""
...
def element_to_feature(element: dict[str, Any]) -> dict[str, Any] | None:
    ...
    properties = {
        "name": first_tag(tags, "name", "brand") or "Unnamed pharmacy",
        "category": "Pharmacy",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch real Munich pharmacy data from OpenStreetMap Overpass."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> int:
    ...
    print(f"Wrote {len(geojson['features'])} pharmacies to {args.output}")
    return 0
