#!/usr/bin/env python3
"""Fetch Munich offices from OpenStreetMap Overpass and write GeoJSON."""
...
(
  node["office"]["office"!="no"]({bbox});
  way["office"]["office"!="no"]({bbox});
  relation["office"]["office"!="no"]({bbox});
...
def office_type(tags: dict[str, Any]) -> str:
    value = first_tag(tags, "office")
    if value:
        return value
    if first_tag(tags, "building") == "office":
...
def to_geojson(overpass_data: dict[str, Any]) -> dict[str, Any]:
    ...
    features.sort(
        key=lambda feature: (
            ...
            str(feature["properties"]["office_type"]).lower(),
...
def default_output_path() -> Path:
    ...
    return (
        ...
        / "data"
        / "city-intelligence"
        / "munich-offices.geojson"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch real Munich office data from OpenStreetMap Overpass."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> int:
    ...
    print(f"Wrote {len(geojson['features'])} offices to {args.output}")
    return 0
